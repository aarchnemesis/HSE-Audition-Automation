 ...
 */
export function buildPendencyDigest(records: HSEDatabaseRecord[]): InspectorPendencyGroup[] {
  ...
  for (const rec of records) {
    ...
    if (!group) {
      group = { inspectorName: rec.inspectorName, role: rec.role, sector: rec.sector, items: [] };
  ...
  return Array.from(groupsByName.values()).sort((a, b) => b.items.length - a.items.length);
}
