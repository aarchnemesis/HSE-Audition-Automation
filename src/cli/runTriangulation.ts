...
const PARKS: ParkRequirement[] = [
  ...
  },
  {
    ...
    clientName: 'Voltalia',
    description:
      'Parque Solar com áreas de média/alta tensão e espaços confinados. Exige NR-33 e LOTO.',
...
async function main() {
  ...
  console.log(
    `   HSE AUDIT AUTOMATION - RELATÓRIO DE APTIDÃO TS (${REF_DATE.toLocaleDateString('pt-BR')})`
  )
  console.log(
    '================================================================================\n'
  )
  ...
  console.log(
    `[DriveFileSystemAdapter] Inspetores processados: ${targetInspectors.length}\n`
  )

  for (const inspector of targetInspectors) {
    console.log(`👤 INSPETOR: ${inspector.name} (${inspector.role})`)
    console.log(
      `   Total de certificados identificados: ${inspector.certificates.size}`
    )
    ...
    for (const park of PARKS) {
      ...
      console.log(
        `   ${'Cód'.padEnd(5)} | ${'Requisito'.padEnd(35)} | ${'Status'.padEnd(16)} | Detalhes`
      )
