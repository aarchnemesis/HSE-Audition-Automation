...
describe('buildRoster', () => {
  ...
  it('inclui pessoa administrativa mesmo sem pasta no Drive, usando dados da RPO', () => {
    const rpo = [
      inspector('JESSICA ALVES DA SILVA', 'ADM', [
        cert('01', new Date(2027, 0, 1)),
      ]),
    ]
  ...
  it('mescla certificados do Drive por cima dos da RPO quando a pessoa tem pasta', () => {
    ...
    const drive = [
      inspector('CARLOS PEREIRA', 'INSP. DE QUALIDADE', [
        cert('01', driveDate),
      ]),
    ]
    ...
    expect(roster[0].inspector.certificates.get('01')?.expirationDate).toEqual(
      driveDate
    )
  })
  ...
  describe('Contrato PJ (código 40) / Aditivo (40.1) — pedido do usuário em 27/08/2026', () => {
    ...
    it('CAMPO + CLT NÃO exige o código 40 — contrato PJ não se aplica', () => {
      const rpo = [
        inspector('TECNICO CLT', 'TO', [], { employmentType: 'CLT' }),
      ]
      const roster = buildRoster(rpo, [])
    ...
    it('quando só existe o Contrato (40), usa o prazo dele', () => {
      ...
      expect(
        roster[0].inspector.certificates.get('40')?.expirationDate
      ).toEqual(contratoDate)
    })
    ...
    it('quando existe Contrato (40) E Aditivo (40.1), o Aditivo substitui o prazo do contrato', () => {
      ...
      const drive = [
        inspector('PILOTO PJ', 'IE', [
          cert('40', contratoDate),
          cert('40.1', aditivoDate),
        ]),
      ]
      const rpo = [inspector('PILOTO PJ', 'IE', [], { employmentType: 'PJ' })]
    ...
    it('mantem a data do RPO quando o colaborador e VALIDADO e a data do RPO e mais recente que o Drive antigo', () => {
    ...
    it('mantem a data do Drive quando o colaborador e PENDENTE_REVISAO mesmo com data mais recente no RPO', () => {
      const driveOldDate = new Date(2024, 5, 10)
      const rpoRenewalDate = new Date(2027, 5, 10)
      const drive = [
      ...
      expect(mergedCert?.expirationDate).toEqual(driveOldDate)
      expect(roster[0].inspector.rpoAuditStatus).toBe('PENDENTE_REVISAO')
    })
  })
})
