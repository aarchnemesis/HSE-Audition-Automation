...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #view-cockpit::-webkit-scrollbar {
      width: 6px;
    }
    #view-cockpit::-webkit-scrollbar-track {
      background: transparent;
    }
    ...
    #view-cockpit::-webkit-scrollbar-thumb:hover {
      background: #94A3B8;
    }

    .cockpit-print-header {
      display: none;
    }
    ...
    .cockpit-segment-group, .cockpit-park-group {
      display: flex;
      align-items: center;
      flex-wrap: wrap;
      gap: 6px;
    }
    .cockpit-seg-btn, .cockpit-park-btn {
      background: #F8FAFC;
      border: 1px solid #CBD5E1;
      border-radius: 6px;
      padding: 4px 10px;
    ...
    .cockpit-seg-btn:hover, .cockpit-park-btn:hover {
      border-color: var(--brand-blue);
      color: var(--brand-blue);
      background: #FFFFFF;
    }
    .cockpit-seg-btn.active {
      background: #0F172A;
      border-color: #0F172A;
      color: #FFFFFF;
    }
    .cockpit-park-btn.active {
      background: #1E293B;
      border-color: #1E293B;
      color: #38BDF8;
    }
    ...
    /* MEDIA PRINT STYLES FOR EXECUTIVE ROSTER & MULTI-PAGE PDF EXPORT */
    @media print {
      @page {
        size: A4 portrait;
...
<body>
  ...
  <main class="main-viewport">
    ...
    <div class="page-title-bar">
      <div>
        ...
        <p id="pageSubheading">Status de mobilização técnica, requisitos básicos e conformidade de campo por colaborador</p>
      </div>
    ...
    <div class="content-body">
      ...
      <div id="view-cockpit" class="tab-view active">
        ...
        <div class="cockpit-print-header">
          ...
          <div class="cockpit-print-date">
            <strong>Relatório de Mobilização</strong><br>
            <span id="cockpitPrintDate">Gerado em: ${new Date().toLocaleDateString('pt-BR')}</span>
          </div>
        </div>
        ...
        <div class="cockpit-kpi-row">
          <div class="cockpit-kpi-card total" onclick="filterCockpitStatus('ALL')" style="cursor:pointer;" title="Ver todos os colaboradores ativos do segmento">
            ...
            <div class="cockpit-kpi-sub" id="cockpitKpiTotalSub">Linha de frente</div>
          </div>
          ...
          <div class="cockpit-kpi-card apto" onclick="filterCockpitStatus('APTO')" style="cursor:pointer;" title="Filtrar colaboradores aptos para mobilização imediata">
            ...
            <div class="cockpit-kpi-sub" id="cockpitKpiAptoSub">Plenos e a vencer</div>
          </div>
        ...
        <div class="cockpit-scope-banner">
          <div class="cockpit-scope-info">
            ...
            <div class="cockpit-scope-desc" id="cockpitScopeDesc">
              Foco: <strong>Linha de Frente de Campo</strong> (65 técnicos/inspetores). Requisitos básicos de mobilização: <strong>ASO (01), GWO BST e NRs regulamentares</strong>. Ausência de módulos específicos (GWO ART, Vestas, Elevador) não gera bloqueio geral.
            </div>
          </div>
          <div class="cockpit-scope-actions">
            ...
            <button class="btn-cockpit-action primary" onclick="printCockpit()" title="Imprimir ou exportar relatório em PDF (Lista Vertical)">
        ...
        <!-- Cockpit Controls Container -->
        <div class="cockpit-controls-container">
          <!-- Row 1: Segment & Park Scope -->
          <div class="cockpit-toolbar-row">
            <div class="cockpit-segment-group">
              <span class="cockpit-filter-label">Segmento:</span>
              <button class="cockpit-seg-btn active" id="cockpitSeg-FRONTLINE" onclick="filterCockpitSegment('FRONTLINE')" title="Técnicos e Inspetores que sobem em aerogerador (IQ, TO, IE)">
                Linha de Frente (<span id="cockpitSegCountFrontline">65</span>)
              </button>
              <button class="cockpit-seg-btn" id="cockpitSeg-LEADERS" onclick="filterCockpitSegment('LEADERS')" title="Líderes de Campo com atuação híbrida (LO)">
                Liderança Híbrida (<span id="cockpitSegCountLeaders">5</span>)
              </button>
              <button class="cockpit-seg-btn" id="cockpitSeg-OFFICE" onclick="filterCockpitSegment('OFFICE')" title="Coordenador, EHS/SSMA e Administrativo (isentos de subida em turbinas)">
                Gestão &amp; Office (<span id="cockpitSegCountOffice">5</span>)
              </button>
              <button class="cockpit-seg-btn" id="cockpitSeg-ALL" onclick="filterCockpitSegment('ALL')" title="Todos os colaboradores da base operacional">
                Todos (<span id="cockpitSegCountAll">75</span>)
              </button>
            </div>

            <div class="cockpit-park-group">
              <span class="cockpit-filter-label">Exigência do Parque:</span>
              <button class="cockpit-park-btn active" id="cockpitPark-BASIC" onclick="filterCockpitParkScope('BASIC')" title="Requisitos Básicos de Mobilização (ASO + GWO BST + NRs)">
                Geral (Básico)
              </button>
              <button class="cockpit-park-btn" id="cockpitPark-ART" onclick="filterCockpitParkScope('ART')" title="Exige GWO ART (Resgate Avançado / AST)">
                Exige GWO ART
              </button>
              <button class="cockpit-park-btn" id="cockpitPark-NR33_RESGATE" onclick="filterCockpitParkScope('NR33_RESGATE')" title="Exige NR-33 Supervisor / Resgate">
                Exige NR-33 Resgate
              </button>
              <button class="cockpit-park-btn" id="cockpitPark-VESTAS" onclick="filterCockpitParkScope('VESTAS')" title="Exige Qualificação Turbinas Vestas (SIT/ESO)">
                Exige Vestas
              </button>
              <button class="cockpit-park-btn" id="cockpitPark-ELEVADOR" onclick="filterCockpitParkScope('ELEVADOR')" title="Exige Elevador JASO (Cremalheira)">
                Exige Elevador
              </button>
            </div>
          </div>
      ...
      <div id="view-rpo" class="tab-view">
        <div class="rpo-summary-row">
          ...
          <div class="rpo-summary-card drive" onclick="filterRpoSubTab('PENDENTE_BACKUP')" style="cursor:pointer;border-color:#C7D2FE;background:#EEF2FF;" title="RPO validado manualmente pela equipe — pendente apenas anexar comprovante no Drive">
            <div class="rpo-summary-val mono" id="rpoKpiPendenteBackup" style="color:#3730A3;">0</div>
            <div class="rpo-summary-label"><svg class="chip-svg" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>Validado RPO (Pendente Backup)</div>
          </div>
        ...
        <div class="card">
          <div class="card-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; flex-wrap:wrap; gap:8px;">
            ...
            <div style="display:flex; gap:6px; flex-wrap:wrap;">
              <button class="chip active" id="rpoChip-ACTIONABLE_DIV" onclick="filterRpoSubTab('ACTIONABLE_DIV')">Divergências de Ação (<span id="rpoChipActionableCount">0</span>)</button>
              <button class="chip" id="rpoChip-PENDENTE_BACKUP" onclick="filterRpoSubTab('PENDENTE_BACKUP')" style="color:#3730A3;background:#EEF2FF;border-color:#C7D2FE;"><svg class="chip-svg" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>Validado RPO (Pendente Backup) (<span id="rpoChipPendenteBackupCount">0</span>)</button>
              <button class="chip" id="rpoChip-ALL_DIV" onclick="filterRpoSubTab('ALL_DIV')">Todas Divergências (<span id="rpoChipDivCount">0</span>)</button>
  ...
  <script>
    ...
    const rpoPendenteBackupCount = rawRpoDivergences.filter(d => d.divergent && d.divergenceKind === 'RPO_VALIDADO_PENDENTE_BACKUP').length;
    const rpoActionableCount = rawRpoDivergences.filter(d => d.divergent && d.divergenceKind !== 'RPO_VALIDADO_PENDENTE_BACKUP').length;
    ...
    rawRpoDivergences.forEach(d => {
      ...
      if (d.divergenceKind === 'DATA_DIVERGENTE') {
        ...
        rpoSomenteRpoCount++;
        rpoNewerCount++;
      } else if (d.divergenceKind === 'RPO_VALIDADO_PENDENTE_BACKUP') {
        rpoNewerCount++;
      }
    });
    ...
    if (document.getElementById('rpoBadgeSidebar')) {
      ...
      badge.innerText = rpoActionableCount;
      badge.title = rpoActionableCount + ' divergências reais de ação (' + rpoPendenteBackupCount + ' validados com backup pendente)';
      badge.style.display = rpoActionableCount > 0 ? 'inline-block' : 'none';
    }
    ...
    if (document.getElementById('rpoKpiActionable')) document.getElementById('rpoKpiActionable').innerText = rpoActionableCount;
    if (document.getElementById('rpoKpiPendenteBackup')) document.getElementById('rpoKpiPendenteBackup').innerText = rpoPendenteBackupCount;
    ...
    if (document.getElementById('rpoChipActionableCount')) document.getElementById('rpoChipActionableCount').innerText = rpoActionableCount;
    if (document.getElementById('rpoChipPendenteBackupCount')) document.getElementById('rpoChipPendenteBackupCount').innerText = rpoPendenteBackupCount;
    ...
    function switchNav(viewKey) {
      ...
      if (heading) {
        if (viewKey === 'cockpit') {
          ...
          if (pageSub) pageSub.innerText = 'Status de mobilização técnica, requisitos básicos e conformidade de campo por colaborador';
    ...
    let cockpitSegment = 'FRONTLINE';
    let cockpitParkScope = 'BASIC';
    let cockpitStatusFilter = 'ALL';
    let cockpitSearchQuery = '';

    function filterCockpitSegment(segment) {
      cockpitSegment = segment;
      document.querySelectorAll('.cockpit-seg-btn').forEach(b => b.classList.remove('active'));
      const btn = document.getElementById('cockpitSeg-' + segment);
      if (btn) btn.classList.add('active');
      renderCockpit(lastFilteredPeople);
    }

    function filterCockpitParkScope(scope) {
      cockpitParkScope = scope;
      document.querySelectorAll('.cockpit-park-btn').forEach(b => b.classList.remove('active'));
      const btn = document.getElementById('cockpitPark-' + scope);
      if (btn) btn.classList.add('active');
      renderCockpit(lastFilteredPeople);
    }
    ...
    function getCollabOperationalHealth(p, parkScope = 'BASIC') {
      ...
      if (isOffice) {
        ...
        const asoRec = p.records.find(r => r.docCode === '01');
        const asoValid = asoRec ? (isConforme(asoRec.statusEHS) || isAVencer(asoRec.statusEHS)) : true;
      ...
      // Habilitações Específicas de Parque (para os cards)
      const rArt = recMap.get('32');
      const hasArt = Boolean(rArt && (isConforme(rArt.statusEHS) || isAVencer(rArt.statusEHS)));
      const rNr33Sup = recMap.get('28');
      const hasNr33Resgate = Boolean(rNr33Sup && (isConforme(rNr33Sup.statusEHS) || isAVencer(rNr33Sup.statusEHS)));
      const rSit = recMap.get('25');
      const rEso = recMap.get('26');
      const hasVestas = Boolean((rSit && (isConforme(rSit.statusEHS) || isAVencer(rSit.statusEHS))) || (rEso && (isConforme(rEso.statusEHS) || isAVencer(rEso.statusEHS))));
      const rElev = recMap.get('31');
      const hasElevador = Boolean(rElev && (isConforme(rElev.statusEHS) || isAVencer(rElev.statusEHS)));
      ...
      requiredCodes.forEach(code => {
        ...
        if (!r) {
          missingItems.push({
            docCode: code,
            docName: docShortNames[code] || ('Doc ' + code),
            statusEHS: 'AUSENTE',
      ...
      const totalRequired = requiredCodes.size;
      const complianceRate = totalRequired > 0 ? Math.round((compliantCount / totalRequired) * 100) : 100;
    ...
    function renderCockpit(peopleList) {
      // 1. Contagens de Segmento sobre a lista completa
      let cntFrontline = 0;
      let cntLeaders = 0;
      let cntOffice = 0;

      peopleList.forEach(p => {
        if (['IQ', 'TO', 'IE'].includes(p.role)) cntFrontline++;
        else if (p.role === 'LO') cntLeaders++;
        else if (['CO', 'ADM', 'EHS'].includes(p.role)) cntOffice++;
      });

      const elSegFrontline = document.getElementById('cockpitSegCountFrontline');
      if (elSegFrontline) elSegFrontline.innerText = cntFrontline;
      const elSegLeaders = document.getElementById('cockpitSegCountLeaders');
      if (elSegLeaders) elSegLeaders.innerText = cntLeaders;
      const elSegOffice = document.getElementById('cockpitSegCountOffice');
      if (elSegOffice) elSegOffice.innerText = cntOffice;
      const elSegAll = document.getElementById('cockpitSegCountAll');
      if (elSegAll) elSegAll.innerText = peopleList.length;

      // 2. Filtrar pelo segmento ativo
      let segmentPeople = peopleList;
      if (cockpitSegment === 'FRONTLINE') {
        segmentPeople = peopleList.filter(p => ['IQ', 'TO', 'IE'].includes(p.role));
      } else if (cockpitSegment === 'LEADERS') {
        segmentPeople = peopleList.filter(p => p.role === 'LO');
      } else if (cockpitSegment === 'OFFICE') {
        segmentPeople = peopleList.filter(p => ['CO', 'ADM', 'EHS'].includes(p.role));
      }

      // 3. Atualizar Banner Explicativo com base no segmento ativo
      const bannerDesc = document.getElementById('cockpitScopeDesc');
      if (bannerDesc) {
        if (cockpitSegment === 'FRONTLINE') {
          bannerDesc.innerHTML = 'Foco: <strong>Linha de Frente de Campo</strong> (' + cntFrontline + ' técnicos/inspetores). Requisitos básicos de mobilização: <strong>ASO (01), GWO BST e NRs regulamentares</strong>. Ausência de módulos específicos (GWO ART, Vestas, Elevador) não gera bloqueio geral.';
        } else if (cockpitSegment === 'LEADERS') {
          bannerDesc.innerHTML = 'Foco: <strong>Liderança de Campo</strong> (' + cntLeaders + ' líderes operacionais). <strong>Atuação híbrida</strong> entre escritório e supervisão em turbina. O planejamento de subida em campo deve observar as reciclagens vigentes.';
        } else if (cockpitSegment === 'OFFICE') {
          bannerDesc.innerHTML = 'Foco: <strong>Gestão &amp; Apoio Administrativo</strong> (' + cntOffice + ' colaboradores: Coordenação, EHS/SSMA, Compras e Comercial). <strong>Isentos de normas de subida/escalada técnica em turbinas</strong>. Monitoramento focado no PCMSO (ASO).';
        } else {
          bannerDesc.innerHTML = 'Visão Consolidada: <strong>Toda a Operação</strong> (' + peopleList.length + ' colaboradores: Linha de frente, liderança híbrida e equipe de gestão/suporte).';
        }
      }

      // 4. Avaliar cada pessoa no segmento com o escopo de parque ativo
      let aptoCount = 0;
      ...
      const evaluatedPeople = segmentPeople.map(p => {
        const health = getCollabOperationalHealth(p, cockpitParkScope);
      ...
      const totalActiveSegment = segmentPeople.length;
      const evaluableCount = totalActiveSegment - isentoCount;
      const aptoRate = evaluableCount > 0 ? Math.round((aptoCount / evaluableCount) * 100) : 100;

      // 5. Atualizar KPIs
      const elTotal = document.getElementById('cockpitKpiTotal');
      if (elTotal) elTotal.innerText = totalActiveSegment;
      const elTotalSub = document.getElementById('cockpitKpiTotalSub');
      if (elTotalSub) {
        if (cockpitSegment === 'LEADERS') elTotalSub.innerText = 'Liderança híbrida';
        else if (cockpitSegment === 'OFFICE') elTotalSub.innerText = 'Gestão & office';
        else if (cockpitSegment === 'ALL') elTotalSub.innerText = 'Base completa';
        else elTotalSub.innerText = 'Linha de frente';
      }
      const elApto = document.getElementById('cockpitKpiApto');
      if (elApto) elApto.innerText = (cockpitSegment === 'OFFICE') ? isentoCount : aptoCount;
      const elAptoRate = document.getElementById('cockpitKpiAptoRate');
      if (elAptoRate) elAptoRate.innerText = aptoRate + '%';
      const elAptoSub = document.getElementById('cockpitKpiAptoSub');
      if (elAptoSub) {
        if (cockpitSegment === 'OFFICE') elAptoSub.innerText = 'Funções de gestão/suporte';
        else elAptoSub.innerText = aptoPlenoCount + ' plenos · ' + alertaCount + ' atenção recert.';
      }
      ...
      // 6. Atualizar contagens dos chips de status
      const chipTotal = document.getElementById('cockpitChipTotalCount');
      if (chipTotal) chipTotal.innerText = totalActiveSegment;
      const chipApto = document.getElementById('cockpitChipAptoCount');
      if (chipApto) chipApto.innerText = (cockpitSegment === 'OFFICE') ? isentoCount : aptoCount;
      ...
      // 7. Filtrar por busca e chip de status
      ...
      const filteredCards = evaluatedPeople.filter(({ p, health }) => {
        ...
        if (cockpitStatusFilter === 'APTO') {
          if (health.status === 'ISENTO_CAMPO') return true;
          if (!health.isMobilizavel) return false;
        }
      ...
      // 8. Ordenar: Bloqueados primeiro, depois Alerta, depois Apto, depois Isento
      const statusOrder = { BLOQUEADO: 1, ALERTA: 2, APTO: 3, ISENTO_CAMPO: 4 };
      ...
      // 9. Atualizar subtítulo de impressão executiva
      const elPrintSub = document.getElementById('cockpitPrintSubtitle');
      if (elPrintSub) {
        const segLabels = {
          FRONTLINE: 'Linha de Frente (' + totalActiveSegment + ' colaboradores)',
          LEADERS: 'Liderança Híbrida (' + totalActiveSegment + ' líderes)',
          OFFICE: 'Gestão & Office (' + totalActiveSegment + ' colaboradores)',
          ALL: 'Todos os Colaboradores (' + totalActiveSegment + ' colaboradores)',
        };
        const parkLabels = {
          BASIC: 'Geral (Básico: ASO + BST + NRs)',
          ART: 'Exige GWO ART',
          NR33_RESGATE: 'Exige NR-33 Resgate',
    ...
    function renderCockpitCard(p, health) {
      ...
      if (health.isOffice) {
        statusBadgeClass = 'badge-cockpit-isento';
        statusLabel = 'GESTÃO / ISENTO DE ESCALADA';
        borderColor = '#94A3B8';
        avatarBg = '#F1F5F9';
        avatarColor = '#475569';
      } else if (health.isLeader) {
        if (health.status === 'BLOQUEADO') {
          ...
          statusBadgeClass = 'badge-cockpit-lider';
          statusLabel = 'LÍDER APTO (HÍBRIDO)';
          borderColor = '#4F46E5';
          avatarBg = '#EEF2FF';
          avatarColor = '#3730A3';
        }
      ...
      let highlightsHtml = '';
      if (health.isOffice) {
        highlightsHtml = '<div class="cockpit-card-ok-msg" style="background:#F8FAFC;border-color:#CBD5E1;color:#475569;"><svg class="ico ico-inline ico-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>Função de escritório/coordenação. Isento de requisitos de subida em turbinas.</div>';
      } else if (health.status === 'BLOQUEADO') {
        ...
        highlightsHtml = '<div class="cockpit-card-pills">' +
          pendencies.map(r => {
            ...
            return '<span class="cockpit-card-pill pill-danger" title="' + r.docName + ' (' + (r.detail || tag) + ')"><svg class="ico ico-inline ico-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>' + docName + ' (' + tag + ')</span>';
          }).join('') +
          '</div>';
      } else if (health.status === 'ALERTA') {
        const pills = [];
      ...
      if (health.isDrone) {
        ...
        const nr33Pill = health.hasNr33Resgate
          ? '<span class="spec-tag qualified" title="Qualificado em NR-33 Supervisor / Resgate"><svg class="ico ico-inline ico-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>NR-33 Resgate OK</span>'
          : '<span class="spec-tag neutral" title="Possui NR-33 Vigia">NR-33 Vigia</span>';

        const vestasPill = health.hasVestas
          ? '<span class="spec-tag qualified" title="Habilitado para Turbinas Vestas (SIT/ESO)"><svg class="ico ico-inline ico-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>Vestas OK</span>'
          : '';

        const elevPill = health.hasElevador
          ? '<span class="spec-tag qualified" title="Habilitado para Elevador JASO"><svg class="ico ico-inline ico-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>Elevador OK</span>'
          : '';

        specsHtml = '<div class="cockpit-card-specs">' + artPill + nr33Pill + (vestasPill ? vestasPill : '') + (elevPill ? elevPill : '') + '</div>';
      }
    ...
    function renderRpoTable() {
      ...
      filtered.slice(0, 200).forEach(item => {
        ...
        } else if (item.divergenceKind === 'RPO_VALIDADO_PENDENTE_BACKUP') {
          badgeHtml = '<span class="diff-badge rpo-validado" style="background:#EEF2FF;color:#3730A3;border:1px solid #C7D2FE;">' + SVG_ICONS.check + 'Pendente Backup Drive</span>';
        ...
        } else if (item.divergenceKind === 'RPO_VALIDADO_PENDENTE_BACKUP' || item.direction === 'RPO_NEWER' || item.divergenceKind === 'SOMENTE_RPO') {
    ...
    function openCollabModal(name) {
      ...
      const isValidated = p.records.some(r => r.rpoAuditStatus === 'VALIDADO');
      const auditBadge = isValidated
        ? ' <span style="display:inline-block;padding:2px 8px;border-radius:12px;font-size:11px;font-weight:700;background:#EEF2FF;color:#3730A3;border:1px solid #C7D2FE;vertical-align:middle;margin-left:8px;">RPO Validado</span>'
        : ' <span style="display:inline-block;padding:2px 8px;border-radius:12px;font-size:11px;font-weight:700;background:#FEF3C7;color:#92400E;border:1px solid #FCD34D;vertical-align:middle;margin-left:8px;">Pendente Revisão</span>';
      document.getElementById('modalCollabName').innerHTML = p.name + ' (' + p.role + ' - ' + p.sector + ')' + auditBadge;
