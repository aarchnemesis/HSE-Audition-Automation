...
export const DEFAULT_STORZ_SELECTORS: StorzSelectors = {
  url:
    process.env.STORZ_URL ||
    `https://storz.sistemaescudo.com.br/admin/main.php?elc=${DEFAULT_STORZ_ELC}`,
...
export class StorzPlaywrightScraper {
  ...
  async runAuditScrape(options?: {
    ...
    if (!user || !pass) {
      log(
        '⚠️ Credenciais (STORZ_USER/STORZ_PASS) não configuradas. Retornando dados salvos no cache/mock.'
      )
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      if (attempt > 1) {
        ...
        log(
          `⏳ Tentativa ${attempt}/${maxRetries} após aguardar ${backoffMs}ms...`
        )
    ...
    log(
      `❌ Todas as ${maxRetries} tentativas falharam. Último erro: ${lastError instanceof Error ? lastError.message : String(lastError)}`
    )
    log(
      '↩️ Retornando dados salvos no cache local (podem estar desatualizados).'
    )
  ...
  private async attemptScrape(
    ...
    try {
      ...
      if (!elcToken) {
        throw new Error(
          'Não foi possível extrair o token "elc" da URL pós-login — necessário para chamar o Dossiê do Aluno.'
        )
      }
      ...
      if (targets.length === 0) {
        log(
          '⚠️ Nenhum targetCollaborators informado — nada a raspar (o fluxo é por pessoa, via lista de Usuários).'
        )
      }
      ...
      for (const collaboratorName of targets) {
        ...
        if (!pessoaId) {
          log(
            `⚠️ Colaborador "${collaboratorName}" não encontrado na lista de Usuários (percorridas todas as páginas).`
          )
          continue
        }
        ...
        for (const course of dossie.courses) {
          ...
          if (!trainingCode) {
            log(
              `⚠️ Não foi possível classificar o código do treinamento para: "${course.turma}". Usando '99' (não classificado).`
            )
            trainingCode = '99'
          }
      ...
      await browser.close()
      log(
        `✅ Raspagem finalizada com sucesso! Total coletado: ${scrapedRequests.length} matrícula(s)/curso(s).`
      )
  ...
  private async login(
    ...
    const passField = page
      .locator(`${this.selectors.loginPassSelector}, input[name="senha"]`)
      .first()
    ...
    const submitBtn = page
      .locator(
        `${this.selectors.loginSubmitSelector}, button[type="submit"], button:has-text("Entrar")`
      )
      .first()
    ...
    await page
      ...
      .catch(() => {
        log(
          '⚠️ Aviso: não detectei a tela de login sumir — pode ter falhado (credenciais inválidas?) ou a tela pós-login ainda não foi mapeada.'
        )
      })
  ...
  private async goToUsuarios(
    page: Page,
    log: (msg: string) => void
  ): Promise<Frame> {
    log('📋 Passo 3: Navegando até Administração → Usuários...')
    await page.hover(this.selectors.administracaoMenuSelector)
    await page.click(this.selectors.usuariosMenuItemSelector, {
      timeout: 10000,
    })

    const frameElement = await page.waitForSelector(
      this.selectors.conteudoIframeSelector,
      { timeout: 15000 }
    )
    const frame = await frameElement.contentFrame()
    if (!frame) {
      throw new Error(
        `Não consegui acessar o conteúdo do iframe "${this.selectors.conteudoIframeSelector}".`
      )
    }

    await frame.waitForSelector(this.selectors.userRowSelector, {
      timeout: 15000,
    })
    return frame
  }
  ...
  private async buildPessoaIdMap(
    ...
    const collectFromCurrentPage = async () => {
      ...
      for (const row of rows) {
        ...
        const name = nameCell
          ? (await nameCell.innerText()).trim().toUpperCase()
          : ''
        ...
        for (const target of targetsUpper) {
          if (name.includes(target) && !found.has(target)) {
            found.set(target, id)
            log(
              `✅ Colaborador "${target}" localizado (pessoa ${id}) na lista de Usuários.`
            )
          }
        }
      }
    ...
    for (
      let pageNum = 2;
      found.size < targetsUpper.size && pageNum <= MAX_PAGES_SAFETY_CAP;
      pageNum++
    ) {
      const pageLink = await frame.$(
        `${this.selectors.pageLinkSelector}:text-is("${pageNum}")`
      )
   ...
   */
  private async fetchDossie(
    page: Page,
    elcToken: string,
    pessoaId: string,
    log: (msg: string) => void
  ): Promise<ParsedDossie | null> {
    try {
      ...
      log(
        `⚠️ Falha ao buscar Dossiê do Aluno para pessoa ${pessoaId}: ${err.message}`
      )
      return null
    }
  }
