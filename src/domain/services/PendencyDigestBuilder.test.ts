...
describe('buildPendencyDigest', () => {
  ...
  it('inclui pessoa com pelo menos um item pendente, mas só os itens pendentes dela', () => {
    const records = [
      makeRecord({ inspectorName: 'FULANO', docCode: '01', statusEHS: 'CONFORME' }),
      makeRecord({ inspectorName: 'FULANO', docCode: '21', statusEHS: 'VENCIDO', detail: 'venceu' })
    ];
  ...
  it('agrupa por pessoa e ordena por quem tem mais pendências primeiro', () => {
    const records = [
      makeRecord({ inspectorName: 'COM_UMA', docCode: '01', statusEHS: 'VENCIDO' }),
      makeRecord({ inspectorName: 'COM_DUAS', docCode: '01', statusEHS: 'VENCIDO' }),
      makeRecord({ inspectorName: 'COM_DUAS', docCode: '21', statusEHS: 'VENCE_07' })
    ];
    const digest = buildPendencyDigest(records);
  ...
  it('trata SOLICITADO_STORZ e STORZ_EM_ANDAMENTO como pendência (não CONFORME)', () => {
    const records = [
      makeRecord({ inspectorName: 'X', docCode: '01', statusEHS: 'SOLICITADO_STORZ' }),
      makeRecord({ inspectorName: 'Y', docCode: '01', statusEHS: 'STORZ_EM_ANDAMENTO' })
    ];
