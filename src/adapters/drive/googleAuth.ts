...
function loadClientCredentials(
  ...
  if (!creds) {
    throw new Error(
      `Formato inesperado em ${credentialsPath}: esperado campo "installed" (Desktop app) ou "web".`
    )
  }
  return creds
}
...
export async function getAuthorizedClient(options?: {
  ...
  console.log(
    `[googleAuth] Token salvo em: ${tokenPath} (as próximas execuções não vão pedir login).`
  )
...
function getClientFromEnv(): OAuth2Client | null {
  ...
  console.log(
    '[googleAuth] Usando credenciais OAuth via variáveis de ambiente (modo CI).'
  )
  return client
}
...
function promptForAuthorizationCode(
  ...
  return new Promise((resolve, reject) => {
    ...
    console.log(
      '  Abra esta URL no navegador e faça login com a conta que tem acesso à pasta:\n'
    )
    console.log(`  ${authUrl}\n`)
    console.log(
      '================================================================================\n'
    ...
    const server = http.createServer((req, res) => {
      ...
      if (error || !code) {
        reject(
          new Error(`Autorização OAuth falhou: ${error || 'código ausente'}`)
        )
      } else {
        resolve(code)
      }
