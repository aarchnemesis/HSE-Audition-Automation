...
async function main() {
  console.log('================================================================================');
  console.log(`   HSE AUDIT AUTOMATION - AUDITORIA TRIPLA (DRIVE + RPO + STORZ) & MAPA TS`);
  ...
  if (targetInspectors[0]) {
    targetInspectors[0].location = { city: 'Mossoró', state: 'RN', lat: -5.1878, lng: -37.3442 };
  }
  if (targetInspectors[1]) {
    targetInspectors[1].location = { city: 'Salvador', state: 'BA', lat: -12.9777, lng: -38.5016 };
  }

  const storzRequests = await storzAdapter.getAllRequests();
  console.log(`[StorzPlaywrightAdapter] Total de solicitações encontradas na Storz: ${storzRequests.length}\n`);

  for (const inspector of targetInspectors) {
    console.log(`👤 INSPETOR: ${inspector.name} (${inspector.role})`);
    console.log(`   📍 Localização Atual/Base: ${inspector.location?.city}/${inspector.location?.state}`);
    console.log(`   Total de certificados no Drive: ${inspector.certificates.size}`);
    console.log('-'.repeat(85));

    for (const park of PARKS_WITH_MODALITY) {
      const auditRes = AuditTriangulator.performTripleAudit(inspector, park, storzRequests, REF_DATE);

      console.log(`\n   🏞️  AVALIAÇÃO PARA: ${auditRes.parkName} (Cliente: ${auditRes.clientName})`);
      console.log(`   📍 Localização do Parque: ${park.location?.city}/${park.location?.state}`);

      console.log('\n   Matriz de Conciliação Tripla (Drive vs RPO vs Storz):');
      console.log(`   ${'Cód'.padEnd(5)} | ${'Requisito'.padEnd(30)} | ${'Status EHS'.padEnd(18)} | Detalhes & Storz`);
      console.log('   ' + '-'.repeat(85));
      ...
      if (auditRes.warningDocsCount > 0 || auditRes.storzPendingCount > 0 || auditRes.expiredDocsCount > 0) {
        ...
        console.log(`   📧 Alerta de e-mail ${emailRes.success ? 'enviado' : 'falhou'}${emailRes.filePath ? ` (${emailRes.filePath})` : ''}`);
      }
    }
  ...
  console.log('================================================================================');
  console.log(' Selecionar Parque Destino: "Parque Solar Sol do Sertão (Oliveira dos Brejinhos/BA)"');
  console.log('\n Status Iluminado no Mapa:');
  console.log('  🟢 Adriano Cirilo Garcia Lima | Base: Salvador/BA   | Status: APTO COM ATENÇÃO (LOTO em andamento Storz) | Distância: ~430 km');
  console.log('  🟢 Adenilço Queiroz da Silva   | Base: Mossoró/RN   | Status: APTO (100% Conforme)                      | Distância: ~980 km');
  console.log(' 👉 Recomendação de Alocação Eficiente: Adriano Cirilo (Menor custo de mobilização)');
  console.log('================================================================================\n');
}
