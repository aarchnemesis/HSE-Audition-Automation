...
export function createDriveAdapter(refDate: Date): IDocumentProvider {
  ...
  if (folderId) {
    ...
    const folderIds = folderId
      ...
      .filter(Boolean)
    console.log(
      `[driveAdapterFactory] Usando GoogleDriveOAuthAdapter (${folderIds.length} pasta(s) raiz configurada(s)).`
    )
  ...
  console.log(
    '[driveAdapterFactory] Usando DriveFileSystemAdapter (path local sincronizado).'
  )
