...
import { Inspector, Certificate, EHSStatus, TrainingModality } from '../models/Certificate.js';
...
export class HSEDatabaseRepository {
  ...
  constructor(dbPath: string = path.join(process.cwd(), 'scratch', 'hse_database.json')) {
    this.dbPath = dbPath;
   ...
   */
  saveAuditSnapshot(auditResults: TripleAuditResult[], inspectors: Inspector[]): void {
    ...
    console.log(`[HSEDatabaseRepository] 💾 Banco de dados HSE atualizado com ${records.length} registro(s) em: ${this.dbPath}`);
  }
  ...
  getAllRecords(): HSEDatabaseRecord[] {
    ...
    try {
      ...
      if (!Array.isArray(parsed)) {
        console.error('[HSEDatabaseRepository] Banco de dados HSE em formato inesperado (esperado array). Ignorando.');
        return [];
      }
      return parsed.filter((r) => typeof r === 'object' && r !== null && typeof r.inspectorName === 'string' && typeof r.docCode === 'string');
    } catch (e) {
      console.error('[HSEDatabaseRepository] Erro ao ler banco de dados HSE:', e);
      return [];
    }
  }
}
