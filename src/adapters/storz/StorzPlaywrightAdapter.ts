...
interface CachedStorzRequest
  extends Omit<
    StorzRequest,
    'requestDate' | 'scheduledDate' | 'completionDate'
  > {
  requestDate: string
...
export class StorzPlaywrightAdapter implements IStorzProvider {
  ...
  private loadCache(): StorzRequest[] {
    if (fs.existsSync(this.cacheFilePath)) {
      try {
        ...
        console.warn(
          '[StorzPlaywrightAdapter] Falha ao ler cache da Storz, criando novo cache.'
        )
      }
    }
    ...
    return [
      {
        ...
        state: 'EM_ANDAMENTO',
        notes:
          'Solicitação criada no portal Storz, aguardando confirmação de turma presencial.',
      },
      {
        id: 'REQ-1002',
  ...
  async getRequestsByCollaborator(
  ...
  async syncWithStorz(
    ...
    const result = await this.scraper.runAuditScrape({
      headless: true,
      targetCollaborators: collaboratorNames,
    })
    if (result.success) {
