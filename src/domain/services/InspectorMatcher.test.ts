...
describe('matchesInspector', () => {
  it('usa CPF como chave exata quando disponível dos dois lados', () => {
    const inspector = makeInspector(
      'ADRIANO CIRILO GARCIA LIMA',
      '123.456.789-00'
    )
    expect(matchesInspector(inspector, 'ADRIANO C. LIMA', '12345678900')).toBe(
      true
    )
  })
  ...
  it('cai para substring de nome quando o CPF não está disponível', () => {
    ...
    expect(
      matchesInspector(
        inspector,
        'ADRIANO CIRILO GARCIA LIMA - Solicitação Storz'
      )
    ).toBe(true)
  ...
  it('ignora diferença de acentuação entre as fontes (bug real: Hamilcar Campos dos Santos Júnior)', () => {
    ...
    expect(
      matchesInspector(inspectorDrive, 'HAMILCAR CAMPOS DOS SANTOS JUNIOR')
    ).toBe(true)

    const inspectorSemAcento = makeInspector(
      'HAMILCAR CAMPOS DOS SANTOS JUNIOR'
    )
    expect(
      matchesInspector(inspectorSemAcento, 'HAMILCAR CAMPOS DOS SANTOS JÚNIOR')
    ).toBe(true)
  })
})
